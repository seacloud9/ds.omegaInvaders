...
engineGlowFS = [
    ...
    "uniform vec2 resolution;",
    "float getGas(vec2 p){",
    "return (cos((p.y*8.0-p.x*8.0+1.0)*PI)+1.0)*0.25;",
    "}",
    "float getSink(vec2 pp){",
    "vec2 p=vec2(pp.x-pp.y,-pp.x-pp.y);",
    "for(int i=1;i<20;i++){",
    "vec2 newp=p;",
    "newp.x+=(0.1/float(i))*sin((float(i)*p.y*8.0+time*0.25+3.0*float(i)+cos((pp.y*2.0+time*0.1)*PI))*PI)*0.3+0.04;",
    "newp.y+=(0.1/float(i))*sin((float(i)*p.x*8.0+time*0.25+3.0*float(i)+cos((pp.x*2.0+time*0.1)*PI))*PI)*0.3-0.04;",
    "p=newp;",
    "}",
    "return getGas(p);",
    "}",
    "void main( void ) {",
    "vec2 position = vec2(mod(gl_FragCoord.x/resolution.x*2.0,1.0),",
    "mod(gl_FragCoord.y/resolution.y,1.0));",
    "vec3 clr=vec3(getSink(position));",
    "clr+=vec3(0.4/(position.y*0.9));",
    "clr+=vec3(0.1/(1.0-position.y));",
    "clr-=mouse.x;",
    "clr*=vec3(0.7,0.1,0.05);",
